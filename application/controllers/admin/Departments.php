...
class Departments extends Admin_Controller
{
    ...
    public function save_departments($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            $teams_id =$this->input->post('teams_id', TRUE);

            // // echo $teams_id;
            // // die();

            $this->department_model->_table_name = "tbl_teams"; // table name
            $this->department_model->_primary_key = "teams_id"; // $id
            //new
                    
            // $where = $this->department_model->array_from_post(array('teams_id')); //input post
            // $check_team = $this->department_model->check_by($where, 'tbl_teams');
            // $check_input_team = $this->department_model->check_by($teams, 'tbl_teams');
            // if (!empty($check_team)) {
            //     $teams_id = $check_team->teams_id;
            //     $actvt = $check_department->deptname;
            // } elseif (!empty($check_input_team)) {
            //     $teams_id = $check_input_team->teams_id;
            //     $actvt = $check_input_team->teams;
            // }
            // else {
                // $team_data['teams'] = $teams;
                // $team_data['departments_id'] = $departments_id;
                if (!empty($teams_id)) {
                
                    $team_id = $teams_id;
                    $this->department_model->save($team_data, $team_id);
                } else {
                
                    $team_id = $this->department_model->save($team_data);
                } 
            //}
    ...
    {
        //new added 
        // $departmentID = $this->db->where('designations_id',$id)->get('tbl_designations')->row()->departments_id;
        $data['teams_info'] = $this->db->get('tbl_teams')->row();
