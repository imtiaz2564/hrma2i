...
// get salary template deatails
    ...
    public function hourly_rateList()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->column_order = array('hourly_grade', 'hourly_rate','days');
    ...
    public function set_hourly_rate($id = null)
    {
        ...
        $edited = can_action('95', 'edited');
        if (!empty($created) || !empty($edited) && !empty($id)) {
            $data = $this->payroll_model->array_from_post(array('hourly_grade', 'hourly_rate', 'days'));
...
// get all expense list by year and month
    ...
    {
        $data['title'] = 'Payroll Filter';
        //$data['test'] = $this->payment_historyMonth('2019-01');
        $data['employee_type'] = $this->input->post('employee_type', true);
        $data['fund_source'] = $this->input->post('fund_source', true);
        $data['team_id'] = $this->input->post('team_id', true);
        $data['experience'] = $this->input->post('experience', true);
        if(!empty($data['employee_type']) && empty($data['fund_source']) && empty($data['team_id']) && empty($data['experience'])){
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($data['employee_type']  ,$data['fund_source'] =null , $data['team_id'] =null , $data['experience'] = null);
        }
        if(!empty($data['fund_source']) && empty($data['employee_type']) && empty($data['team_id']) && empty($data['experience'])){
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($data['employee_type']=null  , $data['fund_source']  , $data['team_id'] =null , $data['experience'] =null);
        }
        if(!empty($data['employee_type']) && !empty($data['fund_source']) && empty($data['team_id']) && empty($data['experience'])){
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($data['employee_type']  , $data['fund_source']  , $data['team_id'] =null , $data['experience'] =null);
        }
        if(!empty($data['team_id']) && empty($data['employee_type']) && empty($data['fund_source']) && empty($data['team_id']) && empty($data['experience'])){
            
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($data['employee_type']=null  , $data['fund_source'] = null , $data['team_id']  , $data['experience'] =null);
        }
        if(!empty($data['employee_type']) && !empty($data['fund_source']) && !empty($data['team_id']) && empty($data['experience'])){
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($data['employee_type']  , $data['fund_source']  , $data['team_id']  , $data['experience'] =null);
        }
        if(!empty($data['experience']) && empty($data['employee_type']) && empty($data['fund_source']) && empty($data['team_id'])){
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($employee_type=null  , $fund_source = null , $team_id = null , $data['experience']);
        }
        if(!empty($data['employee_type']) && !empty($data['fund_source']) && !empty($data['team_id']) && !empty($data['experience'])){
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($data['employee_type']  , $data['fund_source']  , $data['team_id']  , $data['experience'] );
        }
        if(empty($data['employee_type']) && empty($data['fund_source']) && empty($data['team_id']) && empty($data['experience'])){
            $data['employee_payroll'] = $this->payroll_model->get_salary_list($employee_type=null , $fund_source = null , $team_id = null , $experience = null);

        }
        $date = date('Y-m',strtotime("-1 month"));
        
        $salary = $this->payroll_model->get_Last_Month_salary($date);
        $previousTotal = 0;
        $netSalary = 0;
        foreach($salary as $sal){
            $allowance =  $this->payroll_model->get_Last_Month_allowance($sal->salary_payment_id);
            $deduction =  $this->payroll_model->get_Last_Month_deduction($sal->salary_payment_id);
            $basic = $this->payroll_model->get_Last_Basic_Daily($sal->salary_payment_id);
            $netSalary = (double)$basic + ((double)$allowance->totalallowance - (double)$deduction->totaldeduction); 
            $previousTotal = $previousTotal + $netSalary;
           
        }
        $data['total'] = $previousTotal;
        $data['subview'] = $this->load->view('admin/payroll/payroll_filter', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
