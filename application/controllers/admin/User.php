...
class User extends Admin_Controller
{
    ...
    {
        $data = $this->user_model->array_from_post(array('employment_id', 'joining_date', 'contract_ends' , 'contract_no','gob_type', 'cadre_type','itNonIT','gender', 'date_of_birth', 'maratial_status', 'father_name', 'mother_name', 'phone', 'mobile', 'skype', 'present_address', 'passport'));
    ...
    public function save_user()
    {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            if (!empty($check_user) || !empty($check_email)) { // if input data already exist show error alert
                ...
                $profile_data = $this->user_model->array_from_post(array('fullname', 'employment_id', 'company', 'locale', 'employee_type', 'gob_type', 'cadre_type','itNonIT','gender','fund_source','language', 'phone', 'mobile', 'skype', 'designations_id', 'team_id','joining_date','contract_ends','contract_no','reAgreement','education','training','pmoID','undpID','ownCloud','googleGroup','phoneBill','jobLocation','a2imail','unmail','unmail','performanceBonus','securityTest','medicalTest','vendorID','unID','positionNumber','cignaInsurance','sharePoint','level','quartile','gobDesignation','cadreName','cadreBatch','cadreID'));
