    ...
    <form method="post" data-parsley-validate="" novalidate=""
        ...
        <div class="panel panel-custom">
            ...
            <div class="col-sm-9">
                ...
                <div class="form-group">
                <input type="hidden" id= "depId" name="deptId" class="form-control"
                       value=""/>
                       <script>
                             $('#new_departments').change(function () {
                                var dt = $('#new_departments').val();
                                // alert(dt);
                                $('#depId').val(dt);
                             });
                             </script>   
                    
                    <label class="col-lg-4 control-label"><?= lang('select') . ' ' . lang('team') ?>
                        <span
                            class="text-danger">*</span>
                    </label>
                    <div class="col-lg-8">
                        <select class="form-control select_box" style="width: 100%" name="teams_id"
                                id="new_teams">
                            <option value=""><?= lang('new_team') ?></option>

                            <? $dpt = $this->input->post('deptId'); //echo $dpt; 
                            $all_team = $this->db->get('tbl_teams')->result();
                            if (!empty($all_team)) {
                                foreach ($all_team as $v_teams) { ?>
                                    <option <?= (!empty($teams_info->teams_id) && $teams_info->teams_id == $v_teams->teams_id ? 'selected' : null) ?>
                                        value="<?= $v_teams->teams_id ?>"><?php
                                        if (!empty($v_teams->teams)) {
                                            $team = $v_teams->teams;
                                        } else {
                                            $team = lang('undefined_department');
                                        }
                                        echo $team;
                                        //echo  $v_teams->teams_id;
                                        ?></option>
                                    <?php
                                }
                            }
                ...
                <div class="form-group new_teams"
                     style="display: <?= (!empty($teams_info->teams_id) ? 'none' : 'block') ?>">
                    <label class=" col-sm-4 control-label"><?= lang('team') ?></label>
                    <div class="col-sm-8">
                        <input <?= (!empty($teams_info->teams_id) ? 'disabled' : '') ?> type="text" name="teams"  class="form-control new_teams"
                               value=""/>
                    </div>
                </div>
                <div class="form-group">
